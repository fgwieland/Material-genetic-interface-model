...
for jk=jks
    if(~isempty(ar.ple.ps{jk}))
        ...
        conf_lb_point = ar.ple.conf_lb_point(jk);
        conf_ub_point = ar.ple.conf_ub_point(jk);
        yLimit = ylim;
        if ~isinf(conf_lb_point)
            plot([conf_lb_point conf_lb_point], ylim, 'b--')
%             text(conf_lb_point,yLimit(1)+0.2, num2str(conf_lb_point), 'FontSize', 8);
        end
        if ~isinf(conf_ub_point)
            plot([conf_ub_point conf_ub_point], ylim, 'b--')
%             text(conf_ub_point,yLimit(1)+0.2, num2str(conf_ub_point), 'FontSize', 8);
        end
%         
        % hessian CI
